...
class NewsAgencyTest {
    ...
    void testArticleConstructorAndToString() {
        ...
        assertEquals("2019-11-09/1-52-00", article.getPublishingDateTime());
    ...
    void testArticleFactoryAndComparison() {
        ...
        assertEquals("2019-09-17/1-52-00", article.getPublishingDateTime());
        assertEquals(NewsCategory.LIFESTYLE, article.getCategory());

        assertEquals("(LIFESTYLE) Energie: Wasserstoff statt Dieselrauch - veroeffentlicht 09.11.2019 17:52", article.toString());
    ...
    void testNewsAgency() {
        ...
        assertEquals(null, diePresse.getLatestPublishedArticle());
        ...
        Article article = diePresse.getLatestPublishedArticle(); //first article
        assertEquals("(LIFESTYLE) Mitsubishi: Abschied in Grau und Schwarz - veroeffentlicht 09.11.2019 17:52", article.toString());

        article = diePresse.getLatestPublishedArticle(); //no publish - same article should be returned
        assertEquals("(LIFESTYLE) Pick-up: Mit der Pritsche ins Gelände - veroeffentlicht 09.11.2019 17:52", article.toString());

        diePresse.publishArticle();

        article = diePresse.getLatestPublishedArticle(); //second article
        assertEquals("(LIFESTYLE) Energie: Wasserstoff statt Dieselrauch - veroeffentlicht 09.11.2019 17:52", article.toString());
        ...
        while(diePresse.hasMoreArticles()) {
            ...
            article = diePresse.getLatestPublishedArticle();
